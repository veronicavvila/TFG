        ...
        for alpha in alphas:
            for seed in seeds:
                ...
                for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_noisy, y)):
                    ...
                    if ALPHA_ESTIMATION_METHOD == 'robust':
                        alpha_hat = estimar_alpha_robusto(scores_train, S_train, top_q_percent=ALPHA_TOP_Q_PERCENT)
                    else:
                    ...
                    p_y_train_corrected = estimar_probabilidad_real(scores_train, alpha_hat)
                    mi_scores, ranking = calcular_mi_ranking(
                        X_train, p_y_train_corrected, metodo="regresion", random_state=seed
                    )
