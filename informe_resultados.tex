...
\begin{itemize}
    ...
    \item El método robusto de positivos confiables (top 20\%) muestra \textbf{efectividad fuertemente dataset y $\alpha$-dependiente}. Mejora en escenarios con balance moderado (Spambase, Phoneme) a todos los niveles de $\alpha$. En desbalance severo (MiniBooNE con 72\% positivos), el comportamiento es mixto: mejora a $\alpha$ altos (error 36% → 28% para $\alpha=0.5$) pero empeora a $\alpha$ bajos (error 42% → 190% para $\alpha=0.05$), reflejando que la subestimación es síntoma del desbalance, no sesgo uniformemente corregible.
\end{itemize}

\subsection{Conclusión sobre el método de positivos confiables}

El método de positivos confiables (estimación robusta de $\alpha$ usando top 20\% de scores) fue propuesto como solución selectiva para rescatar datasets donde el pipeline PU fallaba por mala estimación del prior $\alpha$. Los resultados validan su \textbf{utilidad selectiva bajo condiciones específicas}:

\begin{itemize}
    \item \textbf{Éxito consistent en balance moderado}: Mejoró significativamente Spambase (error 20% → 16\%) y especialmente Phoneme (error 21.6% → 4.5\%), demostrando que la robustez sí reduce sesgo cuando el balance de clases es razonable (39\%--50\% positivos).
    
    \item \textbf{Comportamiento mixto en desbalance severo}: En MiniBooNE (72\% positivos mayoritarios), el método mejora a $\alpha$ altos (36% → 28%) pero empeora a $\alpha$ bajos (42% → 190%). Esto sugiere que el desbalance extremo crea condiciones donde la subestimación es un síntoma de la asimetría de clases, no un sesgo uniformemente corregible por robustez.
    
    \item \textbf{Recomendación práctica}: Usar cuando: (a) balance de clases es moderado ($\approx 30$--$70\%$), (b) estimación simple de $\alpha$ muestra error consistente ($\geq 15\%$), y (c) dataset tiene suficientes muestras ($n \gtrsim 1000$). Evitar en escenarios de desbalance extremo donde el método PU enfrenta limitaciones estructurales que no pueden solucionarse mejorando $\hat{\alpha}$ únicamente.
\end{itemize}
