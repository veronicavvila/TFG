...
Se evaluó la estimación robusta de $\alpha$ (top 20\%) en Breast Cancer sin ruido. Los resultados muestran que el método robusto \textbf{empeora severamente} la estimación de $\hat{\alpha}$ en todo el rango:

\begin{itemize}
    \item Para $\alpha = 0.05$: $\hat{\alpha}_{\text{media}} = 0.086$ (error 72\%) vs $\hat{\alpha}_{\text{robusto}} = 0.179$ (error 258\%) — empeora 186 puntos porcentuales, sobreestimando más de 3 veces el valor real
    \item Para $\alpha = 0.10$: $\hat{\alpha}_{\text{media}} = 0.161$ (error 61\%) vs $\hat{\alpha}_{\text{robusto}} = 0.347$ (error 247\%) — empeora 186 puntos porcentuales
    \item Para $\alpha = 0.30$: $\hat{\alpha}_{\text{media}} = 0.315$ (error 5\%) vs $\hat{\alpha}_{\text{robusto}} = 0.490$ (error 63\%) — empeora 58 puntos porcentuales
    \item Para $\alpha = 0.50$: $\hat{\alpha}_{\text{media}} = 0.493$ (error 1\%) vs $\hat{\alpha}_{\text{robusto}} = 0.699$ (error 40\%) — empeora 39 puntos porcentuales
    \item El patrón es consistente: el robusto siempre sobrestima y nunca mejora respecto a la media
\end{itemize}

Frente a la estimación basada en la media, el método robusto empeora claramente los resultados, llegando a multiplicar el error incluso en condiciones aparentemente favorables del dataset. Esto sugiere que los ejemplos con mayor score no son representativos de la proporción real de positivos etiquetados, introduciendo un sesgo hacia valores más altos.
% ===================== SONAR =====================
\section{Sonar}
